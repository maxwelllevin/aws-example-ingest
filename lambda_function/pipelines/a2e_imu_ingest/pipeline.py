...
class Pipeline(IngestPipeline):
    ...
    def hook_generate_and_persist_plots(self, dataset: xr.Dataset) -> None:
        ...
        with self.storage._tmp.get_temp_filepath(filename) as tmp_path:
            ...
            roll_label = r"$\.{\theta}_{roll}$ [$\overline{\theta}_r$ =" + f"{mean_roll:.3f} deg]"
            pitch_label = r"$\.{\theta}_{pitch}$ [$\overline{\theta}_p$ =" + f"{mean_pitch:.3f} deg]"

            # Plot the stepped
            ds["roll"].plot.hist(ax=ax, linewidth=2, edgecolor="black", histtype="step", label=roll_label)
            ds["pitch"].plot.hist(ax=ax, linewidth=2, edgecolor="red", histtype="step", label=pitch_label)

            # Set axes and figure labels
            fig.suptitle(f"Buoy Motion Histogram at {location} on {date} from {hhmm1} to {hhmm2}")
